...
void ofApp::loadSettings() {
	...
	if(xml.exists("tolerance")) {
		tracker.setTolerance(xml.getValue("tolerance", 0.99));
	}
...
void ofApp::draw() {
	...
	if(tracker.getFound()) {
		...
		if(bDrawMesh) {
            ...
            //ofMesh faceHeight = tracker.getImageMesh();
            
            ofDrawBitmapString("hello there world-- "+ofToString((int) pos.x) + " " + ofToString((int) pos.y), 200, 200);
            ofNoFill();
            ofDrawRectangle(tracker.getHaarRectangle());
